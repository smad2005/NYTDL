...
define('TORCLI', escapeshellarg('E:\Program Files (x86)\uTorrent\uTorrent.exe'));                                       // Путь к торрент-клиенту
...
if ($torrents) {
  ...
  foreach($torrents as $k => $torname) {
    $linkname = rawurlencode(substr($torname, 0, ($pos = strpos($torname, '(', 5)) ? $pos+1 : 65));
    $html = file_get_contents(LINKSUFFIX.'"'.$linkname.'"', false, $context);
    if (preg_match('~<div class="viewdownloadbutton">\s*<a href="([^"]*tid=(\d+)[^"]*)~', $html, $linkmath)) {
      ...
      copy(html_entity_decode($linkmath[1]), $tmpFl, $context);
      exec(TORCLI.' /DIRECTORY '.$dirname.' '.escapeshellarg($tmpFl));
    } else {
